---
title: "코르그 C1 Air 디지털 피아노"
date: 2026-08-26
lastEdited: 2026-08-26
notionPageId: "3c826dfb-cd79-81c3-b718-fac07834c993"
sidebar:
  order: 9999
---



<img src="/images/코르그-C1-Air-디지털-피아노-0.jpg" alt="" style="max-width: 100%; height: auto; border-radius: 4px; display: block; margin: 10px 0;" />

C1 Air는 2017년 9월에 나온 코르그의 슬림형 콘솔 디지털 피아노임. 상위 모델인 G1 Air와 같은 RH3 건반을 쓰면서 음원과 스피커 구성을 줄여 가격과 크기를 낮춘 모델임. 이름의 Air는 Bluetooth Audio를 뜻하며, 스마트폰 음악을 피아노 스피커로 받아 재생할 수 있음.



<img src="/images/코르그-C1-Air-디지털-피아노-1.png" alt="" style="max-width: 100%; height: auto; border-radius: 4px; display: block; margin: 10px 0;" />


### 건반 / 사운드

C1 Air에는 코르그의 RH3, Real Weighted Hammer Action 3 건반이 들어감. 저음에서 고음으로 갈수록 가벼워지는 4구간 가중 구조이고 터치 감도는 Light, Normal, Heavy, Stable, Fixed 다섯 단계로 바꿀 수 있음. 

RH3의 타건감은 지나치게 무겁거나 가볍지 않은 중간 무게의 액션이고, 2센서임에도 빠른 반복에도 반응이 좋고, 건반 앞쪽과 안쪽의 무게 균형도 비교적 자연스러우며, 타건 소음이 작은 편임. 



<img src="/images/코르그-C1-Air-디지털-피아노-2.jpg" alt="" style="max-width: 100%; height: auto; border-radius: 4px; display: block; margin: 10px 0;" />

흰 건반과 검은 건반에는 인조상아나 합성 흑단 질감 처리가 없고, 에스케이프먼트 재현도 없음. 매끈한 일반 플라스틱 표면이라 마찰감이나 어쿠스틱 피아노식 클릭감이 동급 모델보다 아쉬운 점임. 

피아노 음색은 German Grand와 Japanese Grand 두 개임. German Grand는 둥글고 부드러운 편이고, Japanese Grand는 더 밝은 느낌의 음색임. 

Damper Resonance와 건반을 놓을 때의 소리를 재현하는 Key-off Simulation이 들어지만, 상위 G1 Air에 있는 String Resonance 조절은 없음. 

전체 음색은 30개로, 어쿠스틱 피아노 6개, 전자피아노, 하프시코드와 클라비, 비브라폰과 마림바, 기타, 재즈와 파이프 오르간, 스트링과 합창, 베이스와 피아노 조합으로 구성됨. 최대 동시발음수는 120음이고 Brilliance, Reverb, Chorus를 각각 3단계로 조절할 수 있음.

스피커는 10cm 2개에 25W + 25W 출력임. 전용 스피커 박스가 연주자 쪽으로 소리를 보내는 구조라 동급의 하향식 저출력 스피커보다 저음과 음량이 풍부하고 방을 채우는 힘이 좋다는 평가가 많음. 다만 기본 설정에서는 고역의 선명도가 조금 무르게 들릴 수 있고, 이때 Brilliance를 올리면 보완할 수 있음.



<img src="/images/코르그-C1-Air-디지털-피아노-3.jpg" alt="" style="max-width: 100%; height: auto; border-radius: 4px; display: block; margin: 10px 0;" />


### 디자인 및 조작부

크기는 1346 x 347 x 770mm, 무게는 스탠드와 3페달을 포함해 35kg임. 

건반 덮개에는 소프트 랜딩 장치가 있어 손을 놓아도 천천히 닫힘. RH3 건반 생산은 물론, 전체 조립까지 일본 교토부 난탄시 공장에서 이루어짐.

현재 코르그 공식 페이지 기준 색상은 Black, White, Brown, White Ash, Wooden Black 다섯 가지임. 국내엔 White Ash는 미출시임.

조작부는 건반 왼쪽 위에 볼륨 노브, 기능 버튼, 음색 버튼 10개, 녹음과 재생 버튼, 작은 표시창을 배치함. 자주 쓰는 터치 감도, 밝기, 리버브, 코러스, 음색을 버튼으로 바로 고를 수 있어 앱 없이도 조작하기 쉬운 편임. 



<img src="/images/코르그-C1-Air-디지털-피아노-4.jpg" alt="" style="max-width: 100%; height: auto; border-radius: 4px; display: block; margin: 10px 0;" />

레이어에서는 서로 다른 음색 그룹의 두 음색을 겹치고 음량 균형과 옥타브를 조절할 수 있음. 일반적인 자유 분할 기능은 없고, 왼손 베이스와 오른손 피아노가 미리 조합된 세 가지 Bass & Piano 음색만 제공함. Partner Mode에서는 건반을 같은 음역의 두 구역으로 나눠 교사와 학생이 나란히 연주할 수 있음.

내장 MIDI 녹음은 2트랙, 최대 약 14,000음이며 본체에는 한 곡만 유지함. 왼손과 오른손을 따로 녹음하거나 첫 트랙을 들으며 두 번째 트랙을 덧붙일 수 있음. 40개 피아노 연습곡은 좌우 파트 끄기, 템포 변경, A-B 반복을 지원함.

3페달은 Damper, Soft, Sostenuto 구성이고 Damper와 Soft 페달에서 하프페달을 지원함. Partner Mode에서는 양쪽 연주자가 각각 댐퍼 페달을 쓸 수 있음.

단자는 본체 왼쪽 아래에 LINE OUT, 5핀 MIDI IN/OUT, 3.5mm 헤드폰 2개가 있음. USB 단자는 없어서 컴퓨터나 태블릿과 MIDI를 주고받으려면 별도 USB-MIDI 인터페이스가 필요함. LINE OUT이 따로 있어 외부 앰프나 오디오 인터페이스 연결은 편한 편임.

Bluetooth는 스마트폰 음원을 C1 Air 스피커로 받는 Audio 수신만 지원함. Bluetooth MIDI는 지원하지 않으며 피아노 소리를 무선으로 외부 기기에 보내는 기능도 아님. 헤드폰 사용 시에는 스피커로 들을 때와 비슷한 공간감을 만들려는 Stereo Sound Optimizer가 적용됨.


### 타 브랜드 비교 모델

Yamaha YDP-146 / YDP-S36, Roland RP701 / F701, Kawai CX202


### C1 Air 상세 스펙


| 출시 | 2017년 9월 |
| --- | --- |
| 형태 | 슬림형 콘솔 디지털 피아노 |
| 생산 | 일본 교토부 난탄시 |
| 건반 | RH3, Real Weighted Hammer Action 3 |
| 건반 수 | 88건반, A0-C8 |
| 가중 구조 | 4구간 그레이디드 해머 |
| 터치 감도 | 5단계, Light / Normal / Heavy / Stable / Fixed |
| 건반 표면 | 일반 플라스틱, 인조상아·합성 흑단 처리 없음 |
| 음원 | Stereo PCM |
| 최대 동시발음수 | 120음 |
| 음색 | 30개, 10그룹 x 3뱅크 |
| 주요 피아노 음색 | German Grand / Japanese Grand |
| 피아노 공명 | Damper Resonance / Key-off Simulation |
| 이펙트 | Brilliance / Reverb / Chorus, 각각 3단계 |
| 연주 모드 | Single / Layer / Bass & Piano / Partner |
| 녹음 | 2트랙, 최대 약 14,000음, 사용자 곡 1개 |
| 내장곡 | 음색 데모 10곡 + 피아노 연습곡 40곡 |
| 메트로놈 | 템포 / 박자 / 소리 / 음량 조절 |
| 페달 | Damper / Soft / Sostenuto |
| 하프페달 | Damper와 Soft 지원 |
| 스피커 | 10cm x 2 |
| 출력 | 25W + 25W |
| Bluetooth | Audio 수신, A2DP Sink |
| 연결 단자 | LINE OUT / MIDI IN·OUT / 헤드폰 x 2 / 페달 / DC 24V |
| USB | 없음 |
| 크기 | 1346 x 347 x 770mm, 덮개를 열면 높이 926mm |
| 무게 | 35kg, 스탠드 포함 |
| 소비전력 | 18W |
| 색상 | Black / White / Brown / White Ash / Wooden Black |


### KORG MIDI Data Filer와 C1 Air

C1 Air에는 USB 단자가 없고 본체 녹음 메모리도 한 곡뿐임. KORG MIDI Data Filer를 쓰면 본체에서 녹음한 MIDI 데이터를 Windows PC에 저장하고, 저장한 데이터를 다시 피아노로 보낼 수 있음.

다만 C1 Air와 PC 사이에 별도 USB-MIDI 인터페이스가 필요함. 프로그램은 Windows 7 이후 환경용으로 제공되며, 피아노 설정을 조작하거나 오디오를 녹음하는 앱은 아님.


| 지원 운영체제 | Windows 7 / 8.1 / 10 공식 표기 |
| --- | --- |
| 본체 녹음 데이터 PC 저장 | 지원 |
| PC 데이터 본체 복원 | 지원 |
| 필요 장비 | 별도 USB-MIDI 인터페이스 |
| 오디오 전송·녹음 | 지원하지 않음 |
| 피아노 설정 원격 조작 | 지원하지 않음 |

이 글에 관련해 의견을 나누기 위해 <a href="https://gall.dcinside.com/digitalpiano" target="_blank" style="color: inherit; text-decoration: underline;">dcinside digital piano gallery</a>에도 게시하고 있습니다. 질문이나 토론은 갤러리를 방문해주세요.



<div class="notion-columns" style="--notion-columns:3; display:grid; grid-template-columns:repeat(3, minmax(0, 1fr)); gap:1.5rem; margin:1.5rem 0; align-items:start; width:100%; max-width:100%;">

<div class="notion-column" style="min-width:0; width:100%; max-width:100%; overflow:hidden;">



</div>

<div class="notion-column" style="min-width:0; width:100%; max-width:100%; overflow:hidden;">



<img src="/images/코르그-C1-Air-디지털-피아노-5.png" alt="" style="max-width: 100%; height: auto; border-radius: 4px; display: block; margin: 10px 0;" />



</div>

<div class="notion-column" style="min-width:0; width:100%; max-width:100%; overflow:hidden;">



</div>

</div>

