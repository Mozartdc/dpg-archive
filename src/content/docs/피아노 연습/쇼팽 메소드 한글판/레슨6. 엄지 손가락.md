...
notionPageId: "30426dfb-cd79-8023-a643-ca47652fb2e9"
tags:
  - "쇼팽메소드"
  - "피아노 연습"
sidebar:
...
<strong>1. 건반 감지하기 (Sensing)</strong>
...
<strong>2. 건반 누르기 (Playing the key)</strong>
...
이때 주목할 점은, 엄지를 움직이는 모든 근육의 힘줄이 엄지뿌리(기저부) 근처에 붙어 있다는 사실임. 이 구조 덕분에 우리는 엄지 끝마디(원위 관절)는 힘을 빼고 자유롭게 놔둔 채로, 엄지 전체를 움직여 건반을 누를 수 있는 것임.
...
쉽게 말해, 근육의 길이는 변하지 않은 채로 힘만 꽉 주고 버티는 상태를 말함. 엄지는 움직이거나 무게를 지탱하는 동안에도 근육의 길이를 늘리거나 줄이지 않고, 딱 그 상태로 힘을 주어 관절을 단단하게 고정하는 능력이 필요함.
...
<img src="/images/레슨6.-엄지-손가락-2.png" alt="" style="max-width: 100%; height: auto; border-radius: 4px; display: block; margin: 10px 0;" />
